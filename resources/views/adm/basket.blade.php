    ...
    <table class="table">
        ...
        @for ($i=1; $i<=count($foodsInBasket); $i++)
            <tr>
                ...
                <td>
                    <form action="{{route('adm.basket.sent', $foodsInBasket[$i-1]->id)}}" method="post">
                        @csrf
                        @method('PUT')
                        <button class="btn-outline-danger" type="submit">{{ __('messages.send') }}</button>
                    </form>
                </td>
            </tr>
        @endfor
